...
def total_knn(k, df1, df2, df3, target_dict, main_classes, m_ids, fams, fam_to_id):
    ...
    accuracy1, y_pred1 = run_knn(k, df1, target_dict, main_classes, m_ids, fams, fam_to_id)
    accuracy2, y_pred2 = run_knn(k, df2, target_dict, main_classes, m_ids, fams, fam_to_id)
    accuracy3, y_pred3 = run_knn(k, df3, target_dict, main_classes, m_ids, fams, fam_to_id)

    y_pred_ensemble = [majority_vote([a,b,c]) for a,b,c in zip(y_pred1,y_pred2,y_pred3)]

    accuracy = accuracy_score(fams, y_pred_ensemble)
    return accuracy1, accuracy2, accuracy3, accuracy, y_pred_ensemble