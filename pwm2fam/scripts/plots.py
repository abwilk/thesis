...
from scripts.data_loader import CustomDataset
...
def total_confidence(data, labels, models, k_folds, model_name, save=False, n_fams=20):
    ...
    add_stat_annotation(ax, data=data, x='Group', y='Confidence',
                        box_pairs=[("Correct", "Incorrect")],
...
def IG(m_id, data, labels, m_ids, models, k_folds, model_name, id_to_fam, device, save=False, n_fams=20):   
    ...
    # The plot
    fig, ax = plt.subplots(2, 1, figsize=(8, 5), gridspec_kw={'height_ratios': [10,1]}, sharex=True, constrained_layout=True)
    ...
    if not save:
        ...
        plt.savefig(f'outputs/{model_name}/importance/{m_id}.png')
        plt.close()
...
def make_importance_logo(m_id,gt,importance,ax):
    ...
    # ax[1].bar(x_pos, importance, color='gray')
...
def class_accuracy_comp(id_to_fam_cnts,save=False):
    ...
    # Set horizontal grid lines at every y tick (each category)
    ax.yaxis.set_major_locator(ticker.FixedLocator(x))
    # Set vertical grid lines every 20 units on the x-axis
    ax.xaxis.set_major_locator(ticker.MultipleLocator(20))

    # Enable grid lines for both axes, using a dashed line style and some transparency
